...
<section class="container" style="padding-bottom: 6rem; display: flex; justify-content: center;">
    <div class="glass-panel" style="padding: 3rem; width: 100%; max-width: 600px;">
        ...
        <form method="POST" action="{{ route('dashboard.modules.store') }}" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 1.5rem;">
            @csrf
            <div>
                <label for="title" style="display: block; margin-bottom: 0.5rem; color: var(--text-muted);">Judul Modul (Misal: Eye Level)</label>
                <input type="text" id="title" name="title" value="{{ old('title') }}" required autocomplete="off"
            ...
            <div>
                ...
                <textarea id="description" name="description" required rows="5"
                    style="width: 100%; padding: 1rem; border-radius: 8px; border: 1px solid var(--glass-border); background: rgba(15, 23, 42, 0.5); color: white; font-family: var(--font-main); outline: none; resize: vertical;">{{ old('description') }}</textarea>
            </div>

            <div>
                <label for="image" style="display: block; margin-bottom: 0.5rem; color: var(--text-muted);">Gambar Ilustrasi (Opsional tapi disarankan)</label>
                <input type="file" id="image" name="image" accept="image/*"
                    style="width: 100%; padding: 1rem; border-radius: 8px; border: 1px solid var(--glass-border); background: rgba(15, 23, 42, 0.5); color: white; font-family: var(--font-main); outline: none;">
            </div>

            <button type="submit" class="btn btn-primary" style="width: 100%; margin-top: 1rem;">Simpan Modul</button>
        </form>
    </div>
</section>
@endsection
